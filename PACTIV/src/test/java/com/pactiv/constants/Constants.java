package com.pactiv.constants;

/**
 * The Class Constants.
 */

public class Constants {


	/** The Constant HTML. */
	public static final String HTML = "html";

	/** The Constant COLON. */
	public static final String COLON = ":";

	/** The Constant STEP_DEFINITIONS. */
	public static final String STEP_DEFINITIONS = "com.pactiv.steps";

	/** The Constant FEATURES_PATH. */

	public static final String FEATURES_PATH = "src/test/resources/features/PDP.feature";
	
	/** The Constant ASSET_APIURL. */

	public static final String ASSET_APIURL = "https://bridgeviewfaistaging.pactiv.com:8443/Thingworx/Things/";
	
	/** The Constant PART_APIURL. */

	public static final String PART_APIURL = "/Services/getProcessSheetValues?asset=";
	
	/** The Constant THERMOFORMER1_APIURL. */
	                                      
	public static final String THERMOFORMER1_APIURL = "https://bridgeviewfaiqa.pactiv.com:8443/Thingworx/Things/Line1/Services/getProcessSheetValues?asset=Thermoformer1";
	
	/** The Constant TRIMPRESS1_APIURL. */

	public static final String TRIMPRESS1_APIURL = "https://bridgeviewfaiqa.pactiv.com:8443/Thingworx/Things/Line1/Services/getProcessSheetValues?asset=TrimPress1";
	
	/** The Constant EXTRUDER13B_APIURL. */

	public static final String EXTRUDER13B_APIURL = "https://bridgeviewfaistaging.pactiv.com:8443/Thingworx/Things/Line13/Services/getProcessSheetValues?asset=Extruder13B";
	

	/** The Constant END_TO_END_FEATURES_PATH. */
	public static final String END_TO_END_FEATURES_PATH = "src/test/resources/features/LoginFunctionality.feature";
	
	/** The Constant PASS. */
	public static final String PASS = "Pass";

	/** The Constant FAIL. */
	public static final String FAIL = "Fail";

	/** The Constant UNDERSCORE. */
	public static final String UNDERSCORE = "_";

	/** The Constant MM_dd_yyyy. */
	public static final String MM_dd_yyyy = "MM/dd/yyyy";

	/** The Constant APPURL. */
	public static final String APPURL = "APPURL";
	
	/** The Constant STAGURL. */
	public static final String STAGURL = "STAGURL";
	
	public static final String DMGURL =  "DMGURL";
	
	/** The Constant TRUE. */
	public static final String TRUE = "TRUE";

	/** The Constant BASIC. */
	public static final String BASIC = "Basic";

	/** The Constant CONFIG_PROPERTY. */
	public static final String CONFIG_PROPERTY = "\\src\\test\\resources\\config\\config.property";
	
	/** The Constant HANDLE_POPUP. */
	public static final String HANDLE_POPUP = "HANDLE_POPUP";

	/** The Constant TIME_OUT_DURATION. */
	public static final String TIME_OUT_DURATION = "TIME_OUT_DURATION";

	/** The Constant USER_DIR. */
	public static final String USER_DIR = "user.dir";

	/** The Constant OPEN_BRACES. */
	public static final String OPEN_BRACES = "{";

	/** The Constant CLOSE_BRACES. */
	public static final String CLOSE_BRACES = "}";

	/** The Constant ROUND_OPEN_BRACES. */
	public static final String ROUND_OPEN_BRACES = ")";

	/** The Constant ROUND_CLOSED_BRACES. */
	public static final String ROUND_CLOSED_BRACES = ")";

	/** The Constant QUOTES. */
	public static final String QUOTES = "\"";

	/** The Constant STATUS. */
	public static final String STATUS = "status";

	/** The Constant DATE_FORMAT_YYYY_MM_DD. */
	public static final String DATE_FORMAT_YYYY_MM_DD = "yyyy/MM/dd";

	/** The Constant TIME_STAMP_FORMAT_DD_MM_YYYY_HH_MM_SS. */
	public static final String TIME_STAMP_FORMAT_DD_MM_YYYY_HH_MM_SS = "dd-MM-yyyy HH mm ss";

	/** The Constant DOUBLE_FORWARD_SLASH. */
	public static final String DOUBLE_FORWARD_SLASH = "\\";

	/** The Constant TEST_RESULTS. */
	public static final String TEST_RESULTS = "testResults";

	/** The Constant HYPHEN. */
	public static final String HYPHEN = "-";

	/** The Constant SPACE. */
	public static final String SPACE = " ";

	/** The Constant GET. */
	public static final String GET = "GET";

	/** The Constant POST. */
	public static final String POST = "POST";

	/** The Constant PUT. */
	public static final String PUT = "PUT";

	/** The Constant ACCEPT. */
	public static final String ACCEPT = "Accept";

	/** The Constant APPLICATION_JSON. */
	public static final String APPLICATION_JSON = "application/json";

	/** The Constant ID. */
	public static final String ID = "id";

		/** The Constant EXECUTIONS. */
	public static final String EXECUTIONS = "executions";

	/** The Constant EXCEL. */
	public static final String EXCEL = "excel";

	/** The Constant LOGS. */
	public static final String LOGS = "logs";

	/** The Constant QUESTION_MARK. */
	public static final String QUESTION_MARK = "?";

	/** The Constant SCREENSHOTS. */
	public static final String SCREENSHOTS = "screenshots";

	/** The Constant EXTENT_REPORT. */
	public static final String EXTENT_REPORT = "extentreport";

	/** The Constant EXECUTE. */
	public static final String EXECUTE = "/execute";
	/** The Constant EQUALS. */
	public static final String EQUALS = "=";

	/** The Constant FORWARD_SLASH. */
	public static final String FORWARD_SLASH = "/";

	/** The Constant PROJECT_ID. */
	public static final String PROJECT_ID = "projectId";

	/** The Constant VERSION_ID. */
	public static final String VERSION_ID = "versionId";

	/** The Constant CYCLE_ID. */
	public static final String CYCLE_ID = "cycleId";

	/** The Constant AMPERSAND. */
	public static final String AMPERSAND = "&";

	/** The Constant YES. */
	public static final String YES = "YES";

	/** The Constant NO. */
	public static final String NO = "NO";

	/** The Constant NEWLINE. */
	public static final String NEWLINE = "/n";

	/** The Constant CUCUMBER_EXTENT_REPORT_FORMATTING. */
	public static final String CUCUMBER_EXTENT_REPORT_FORMATTING = "com.cucumber.listener.ExtentCucumberFormatter";

	/** The Constant Dollar. */
	public static final String Dollar = "$";
	
	/** The Constant PACTIV_PRODUCTIONTRACKING_OBJECTREPOSITORY_PROPERTY. */
	public static final String PACTIV_PRODUCTIONTRACKING_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\ProductionTracking.property";
	
	
	/** The Constant PACTIV_LOGIN_OBJECTREPOSITORY_PROPERTY. */
	public static final String PACTIV_LOGIN_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\PactivLogin.property";
	
	/** The Constant PACTIV_LINEVIEW_OBJECTREPOSITORY_PROPERTY. */
	public static final String LINEVIEW_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\LineView.property";
	
	/** The Constant PACTIV_REPORTLOG_OBJECTREPOSITORY_PROPERTY. */
	public static final String PACTIV_REPORTLOG_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\ReportLog.property";
	
	/** The Constant PACTIV_UPLOADPROCESSSHEET_OBJECTREPOSITORY_PROPERTY. */
	public static final String PACTIV_UPLOADPROCESSSHEET_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\UploadProcessSheet.property";
	
	/** The Constant PACTIV_UPLOADPROCESSSHEET_OBJECTREPOSITORY_PROPERTY. */
	public static final String PACTIV_CONTROLTOWER_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\ControlTower.property";
	
	/** The Constant PACTIV_ASSETABRIDGED_OBJECTREPOSITORY_PROPERTY. */
	public static final String PACTIV_ASSETABRIDGED_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\AssetAbridged.property";
	
	/** The Constant PACTIV_BREADCRUMB_OBJECTREPOSITORY_PROPERTY. */
	public static final String PACTIV_BREADCRUMB_OBJECTREPOSITORY_PROPERTY = "\\src\\test\\resources\\properties\\BreadCrumb.property";
	
	public static final String PACTIV_CPT_DMG_OBJECTREPOSITORY_PROPERTY = "\\\\src\\\\test\\\\resources\\\\properties\\\\DMGApp.property";
	 
		/** The Constant LOG_FILE_NAME. */
	// Test data Path
	public static final String LOG_FILE_NAME = "log.log";

	/** The Constant EXTENT_REPORT_CONFIG_XML. */
	public static final String EXTENT_REPORT_CONFIG_XML = "\\src\\test\\resources\\properties\\config.xml";

	/** The Constant ZAPI_FILE_NAME. */
	public static final String ZAPI_FILE_NAME = "zapi.txt";

	/** The Constant EXCEL_FILE_NAME. */
	public static final String EXCEL_FILE_NAME = "excel.xlsx";

	/** The Constant ProcessSheet_DATA_PATH. */
	// TestReport Path
	public static final String PROCESSSHEET_DATA_PATH = "\\src\\test\\resources\\processSheets\\";
	
	/** The Constant TEST_DATA_PATH. */
	// TestReport Path
	public static final String TEST_DATA_PATH = "\\src\\test\\resources\\testdata\\testData.xlsx";

	/** The Constant LOG4J_FILE_PATH. */
	// LOG4J path
	public static final String LOG4J_FILE_PATH = "\\src\\test\\resources\\config\\log4j.properties";

	/** The Constant LOG_FILE_NAME_ATTRIBUTE. */
	public static final String LOG_FILE_NAME_ATTRIBUTE = "logFileName.log";

	/** The Constant CONFIG_AUTHORIZATION. */
	// config authorizations
	public static final String CONFIG_AUTHORIZATION = "AUTHORIZATION";

	// Driver Paths
	/** The Constant CHROME_DRIVER_PATH. */
	// Chrome
	public static final String CHROME_DRIVER_PATH = "/src/test/resources/drivers/chromedriver.exe";

	/** The Constant CHROME_WEBDRIVER. */
	public static final String CHROME_WEBDRIVER = "webdriver.chrome.driver";

	/** The Constant GEKO_DRIVER. */
	// Mozilla
	// public static final String GEKO_DRIVER = "webdriver.gecko.driver";
	public static final String GEKO_DRIVER = "webdriver.firefox.marionette";
	/** The Constant GEKO_DRIVER_PATH. */
	public static final String GEKO_DRIVER_PATH = "/src/test/resources/drivers/geckodriver.exe";

	/** The Constant IE_DRIVER. */
	// IE
	public static final String IE_DRIVER = "webdriver.ie.driver";

	/** The Constant IE_DRIVER_PATH. */
	public static final String IE_DRIVER_PATH = "/src/test/resources/drivers/iedriver.exe";

	/** The Constant EDGE_DRIVER. */
	// IE_EDGE
	public static final String EDGE_DRIVER = "webdriver.edge.driver";

	/** The Constant ANDROID_CHROME_PATH. */
	public static final String ANDROID_CHROME_PATH = "/src/test/resources/drivers/mobilechromedriver.exe";

	/** The Constant ANDROID_CHROME_WEBDRIVER. */
	public static final String ANDROID_CHROME_WEBDRIVER = "webdriver.chrome.driver";

	/** The Constant EDGE_PATH. */
	public static final String EDGE_PATH = "/src/test/resources/drivers/edgedriver.exe";

	/** The Constant SCENARIO_ID. */
	// Test data constants
	public static final String SCENARIO_ID = "Scenario_Id";

	/** The Constant TEST_ID. */
	public static final String RESULTS = "Test Results";

	/** The Constant TEST_ID. */
	public static final String TEST_ID = "Test_Id";

	/** The Constant STEP_RESULT. */
	public static final String STEP_RESULT = "Step_Result";

	/** The Constant TEST_RESULT. */
	public static final String TEST_RESULT = "Test_Result";

	/** The Constant TEST_RESULTS_REPORT. */
	public static final String TEST_RESULTS_REPORT = "TestResults_Report";

	// Browsers
	/** The Constant SAFARI. */
	public static final String SAFARI = "safari";

	/** The Constant IE. */
	public static final String IE = "ie";

	/** The Constant CHROME. */
	public static final String CHROME = "chrome";

	/** The Constant MOZILLA_FIREFOX. */
	public static final String MOZILLA_FIREFOX = "firefox";

	/** The Constant EDGE. */
	public static final String EDGE = "edge";
	
   // public static final string LINE LEVEL REPORTS = "line level reports"



}
